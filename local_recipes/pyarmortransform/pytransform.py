# -*- coding: utf-8 -*-
# @Author: lnorb.com
# @Date:   2022-02-26 13:53:45
# @Last Modified by:   lnorb.com
# @Last Modified time: 2022-07-06 14:03:11
...
def path_info():
    ...
    print_content(current.parent)
    print_content(current.parent.parent)
    print_content(current.parent.parent.parent)
...
def _load_library(path=None, is_runtime=0, platid=None, suffix="", advanced=0):
    ...
    for path in os.environ["PATH"].split(os.pathsep):
        print_content(Path(path))
    print("CWD:")
    print(os.getcwd())

    plat = platform.system().lower()
    print(f"PYTRANSFORM PLATFORM: {plat}")
    name = "libpytransform" + suffix
    print("unIqueStr")
    if plat == "linux":
        filename = name + ".so"
        print(f"LINUX DETECTED, SETTING TO {filename}")
    elif plat == "darwin":
        ...
        raise PytransformError("PYTRANSFORM  Platform %s not supported" % plat)

    print(f"PYTRANSFORM ABSPATH: {filename}")
    print(f"PYTRANSFORM platid: {platid}")
    ...
    #     raise PytransformError('PYTRANSFORM Could not find "%s"' % filename)

    try:
        m = cdll.LoadLibrary(filename)
    except Exception as e:
        print("PYTRANSFORM Load %s failed:\n%s" % (filename, e))
        raise
...
def pyarmor_runtime(path=None, suffix="", advanced=0):
    sys.PYARMOR_LICENSE = "/Users/orb/license.lic"
    print("SETTING LICENSE TO /Users/orb/license.lic")
    # sys.PYARMOR_LICENSE = os.path.expanduser('~/license.lic')
