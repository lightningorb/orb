...
def get_docs_for_click(
    *,
    ...
    docs = f".. _{title.replace(' ', '-').replace('`', '').replace('_', '-')}:\n\n"
    docs += f"{title}\n"
    docs += "=" * len(title)
    docs += "\n\n"
    ...
    if indent == 0:
        ...
        with open("docs/source/cli/cli_toc.rst", "w") as f:
            f.write(ref)
    ...
    if isinstance(obj, Group):
        ...
        for command in commands:
            ...
            cmd_docs = get_docs_for_click(
                obj=command_obj, ctx=ctx, indent=indent + 1, call_prefix=use_prefix
            )
            if indent == 1:
                cmd_name = f"{use_prefix.replace(' ', '_')}_{command}"
                with open("docs/source/cli/cli_toc.rst", "a") as f:
                    f.write(f"   cli/{cmd_name}\n")
                with open(f"docs/source/cli/{cmd_name}.rst", "w") as f:
                    f.write(cmd_docs)
            docs += cmd_docs
    return docs
